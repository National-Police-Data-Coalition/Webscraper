...
class FiftyAIncidentParser(ParserMixin):
    ...
    def _get_stop_type(self, details: list[str]) -> str | None:
        reason = next(
            (item for item in details if re.search(re.escape('Reason for contact:'), item)),
            None,
        )
        if reason:
    ...
    def _parse_victim(self, soup):
        ...
        if complainant:
            ...
            if not complainant_text:
                return None
            age = (
                complainant.find('span', class_='age').get_text(strip=True)
                if complainant.find('span', class_='age')
                else None
            )
            complainant_text = (
                complainant_text.replace(age, '').replace(',', '').strip() if age else complainant_text.strip()
            )
            ...
            return {'ethnicity': ethnicity, 'gender': gender, 'age': age}
        return None
    ...
    def parse_complaint(self, complaint_html: str, complaint_link: str) -> dict | None:
        ...
        return {
            ...
            'time_of_incident': datetime.strptime(details[0].replace('Incident: ', ''), '%B %d, %Y').strftime(
                '%m-%d-%Y %H:%M:%S'
            ),
            'description': f'Incident scraped from: {complaint_link}',
            'location': f'{incident_location} In NYPD {precinct_number} Precinct {precinct_name}'
            ...
            'agencies_present': [
                f'NYPD {precinct_number} Precinct {precinct_name}' if precinct_number and precinct_name else None
            ],
